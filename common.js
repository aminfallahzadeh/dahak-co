...
links.forEach((item) => {
  item.addEventListener("click", () => {
    if (item.id == "web-gis") {
      main.setAttribute("include-html", "pages/webgis.html");
    }
    if (item.id == "geobi") {
      main.setAttribute("include-html", "pages/geobi.html");
    }
    if (item.id == "fsm") {
      main.setAttribute("include-html", "pages/fsm.html");
    }
    if (item.id == "senama") {
      main.setAttribute("include-html", "pages/senama.html");
    }
    if (item.id == "twin") {
      main.setAttribute("include-html", "pages/twin.html");
    }
    if (item.id == "system-production") {
      main.setAttribute("include-html", "pages/system-production.html");
    }
    if (item.id == "datascan") {
      main.setAttribute("include-html", "pages/datascan.html");
    }
    if (item.id == "gather-info") {
      main.setAttribute("include-html", "pages/gather-info.html");
    }
    if (item.id == "man") {
      main.setAttribute("include-html", "pages/man.html");
    }
    if (item.id == "counceling") {
      main.setAttribute("include-html", "pages/counceling.html");
    }
    if (item.id == "geomarketing") {
      main.setAttribute("include-html", "pages/geomarketing.html");
    }
    if (item.id == "gis") {
      main.setAttribute("include-html", "pages/gis.html");
    }
    if (item.id == "map") {
      main.setAttribute("include-html", "pages/map.html");
    }
    includeHTML();
  });
});
...
document.addEventListener("scroll", () => {
  ...
  const mobileNavMenuList = document.querySelectorAll(
    ".navigation__mobilenav--link"
  );
  ...
  if (window.scrollY > 0) {
    ...
    dropdown.forEach(
      (element) => (element.style.backgroundColor = "rgba(0,0,0,0.8)")
    );
    ...
    document
      .querySelectorAll(".navigation__list--link")
      .forEach((element) => (element.style.color = "#4b4b4b"));
    logo.src = "img/logo.png";
    dahak.style.backgroundImage =
      "linear-gradient(to left, #f5a08c, #2a969d, #40251f)";
    dropdown.forEach(
      (element) => (element.style.backgroundColor = "rgba(255, 255, 255, .6)")
    );
